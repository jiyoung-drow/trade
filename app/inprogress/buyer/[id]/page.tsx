    ...
    if (id) {
      ...
      const unsubscribeSnapshot = onSnapshot(docRef, async (docSnap) => {
        if (docSnap.exists()) {
          ...
          // 승인 시 구매자 보유금 차감 및 자동 정산 처리
          if (data.status === "완료" && !data.buyerSettled) {
            const settlementAmount = data.settlementAmount || 0;
            const buyerRef = doc(db, "users", data.participantId);
            await updateDoc(buyerRef, {
              balance: data.participantBalance
                ? data.participantBalance - settlementAmount
                : 0,
            });
            await updateDoc(docRef, { buyerSettled: true });
          }
    ...
    <div className="max-w-md mx-auto p-4 space-y-4">
      ...
      <div>
        ...
        {application.sentNicknames && application.sentNicknames.length > 0 ? (
          application.sentNicknames.map((n: any, idx: number) => (
